...
const FAQCard: React.FC<Props> = ({ title, description }) => {
  ...
  return (
    <div className="FAQCard glassCard max-w-[1076px] p-4 overflow-hidden">
      ...
      >
        <h2 className="text-left pl-4 grayTextGradient text-[20px] lg:text-[24px]">{title}</h2>
      ...
      {/* bottom section - visible when expanded */}
      {/* {isExpanded && ( */}
      <div
        className="mt-2 ml-4"
        style={{
      ...
      >
        <div className="min-h-20">
          <p className="pt-6 text-white/60 text-[14px]">{description}</p>
        </div>
      </div>
      {/* )} */}
    </div>
